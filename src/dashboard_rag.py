...
def crear_dashboard_evaluacion(
    ...
    if metric_descriptions is None:
        metric_descriptions = {
            "faithfulness": "Mide si la respuesta del LLM es consistente con la información proporcionada en el contexto recuperado. Un valor alto indica que el LLM no 'alucina'.",
            "answer_relevancy": "Evalúa la relevancia de la respuesta del LLM a la pregunta formulada, sin importar el contexto. Mide si la respuesta va al grano.",
            "context_precision": "Mide la precisión del contexto recuperado. Un valor alto indica que todos los fragmentos recuperados son relevantes para la pregunta.",
            "context_recall": "Mide la exhaustividad del contexto recuperado. Un valor alto indica que todos los fragmentos necesarios para responder a la pregunta (según la verdad fundamental) han sido recuperados."
        }

    # Asegurarse de que la columna 'model_name' exista. Si no, asignarle un nombre por defecto.
    if 'model_name' not in df.columns or df['model_name'].isnull().all():
        if models_to_compare and len(models_to_compare) == 1:
            df['model_name'] = models_to_compare[0]
        else:
            df['model_name'] = "Modelo Único"
    
    if models_to_compare is None or not models_to_compare:
        models_to_compare = df['model_name'].unique().tolist()
        if not models_to_compare: # Si aún está vacío, fallback
            models_to_compare = ["Modelo Único"]
            df['model_name'] = models_to_compare[0]
            
    # Asegurarse de que la columna 'timestamp' sea de tipo datetime para gráficos temporales
    if 'timestamp' not in df.columns:
        df['timestamp'] = pd.to_datetime('now') # Añadir timestamp si falta, para gráficos temporales
    ...
    avg_faithfulness_per_model = df.groupby('model_name')['faithfulness'].mean().fillna(0) # Manejar NaNs en el promedio
    ...
    <head>
        ...
        <style>
            body {{ font-family: 'Roboto', sans-serif; margin: 20px; background-color: #fcfbf8; color: #36454F; }}
            h1 {{ font-size: 32pt; text-align: center; color: #5c4033; margin-bottom: 30px; border-bottom: 2px solid #a17a6a; padding-bottom: 15px; }}
            h2 {{ font-size: 24pt; color: #7e6357; margin-top: 40px; border-bottom: 1px solid #dcdcdc; padding-bottom: 10px; }}
            h3 {{ font-size: 18pt; color: #7e6357; margin-top: 30px; }}
            .section {{ background-color: #f5f2ed; border-radius: 8px; box-shadow: 0 2px 4px rgba(0,0,0,.1); padding: 20px; margin-bottom: 25px; }}
            .kpi-container {{ display: flex; justify-content: space-around; flex-wrap: wrap; margin-bottom: 20px; }}
            .kpi-box {{ background-color: #e8e2d7; border-radius: 5px; padding: 15px 20px; margin: 10px; text-align: center; flex: 1; min-width: 200px; max-width: 28%; box-shadow: inset 0 0 5px rgba(0,0,0,.05); }}
            .kpi-label {{ font-size: 14pt; color: #696969; margin-bottom: 5px; }}
            .kpi-value {{ font-size: 24pt; font-weight: 700; color: #a0522d; }}
            .metric-description {{ font-size: 12pt; line-height: 1.6; color: #696969; margin-bottom: 10px; }}
            .interactive-tip {{ background-color: #e0efe0; border-left: 5px solid #6b8e23; padding: 15px; margin: 20px 0; font-size: 13pt; color: #36454F; }}
            table {{ width: 100%; border-collapse: collapse; margin-top: 20px; }}
            th, td {{ border: 1px solid #d3d3d3; padding: 8px; text-align: left; vertical-align: top; }}
            th {{ background-color: #e8e2d7; font-weight: bold; color: #5c4033; }}
            .dataframe-table {{ font-size: 11pt; }}
            details {{ margin-top: 20px; background-color: #f5f2ed; border: 1px solid #d3d3d3; border-radius: 5px; padding: 10px; }}
            summary {{ font-weight: bold; cursor: pointer; color: #7e6357; font-size: 16pt; }}
            ...
            .model-summary-card h3 {{ margin-top: 0; margin-bottom: 15px; color: #5c4033; font-size: 20pt; }}
    ...
    for model in models_to_compare:
        ...
        if not df_model.empty:
            avg_metrics = df_model[['faithfulness', 'answer_relevancy', 'context_precision', 'context_recall', 'latency']].mean().fillna(0) # Manejar NaNs en el promedio
            html_content += f"""
    ...
    """
    display_df_cols = ['timestamp', 'model_name', 'question_category', 'question', 'answer', 'ground_truth', 'contexts',
                     'faithfulness', 'answer_relevancy', 'context_precision', 'context_recall',
                     'latency', 'answer_tokens', 'simulated_satisfaction']
    
    # Filtra las columnas que realmente existen en el DataFrame
    existing_display_df_cols = [col for col in display_df_cols if col in df.columns]
    display_df = df[existing_display_df_cols].copy()

    if 'timestamp' in display_df.columns:
        display_df['timestamp'] = display_df['timestamp'].dt.strftime('%Y-%m-%d')
    else:
        display_df.insert(0, 'timestamp', pd.NA) # Insertar columna vacía si no existe

    # Asegurarse de que las columnas de ordenación existan antes de ordenar
    sort_cols = [col for col in ['question', 'timestamp', 'model_name'] if col in display_df.columns]
    if sort_cols:
    ...
    if 'simulated_satisfaction' in display_df.columns:
        display_df['simulated_satisfaction'] = display_df['simulated_satisfaction'].apply(lambda x: '✅ Satisfactorio' if x == 1 else ('❌ Insatisfactorio' if x == 0 else 'N/A'))


    if 'contexts' in display_df.columns:
        display_df['contexts'] = display_df['contexts'].apply(
            lambda x: "<br>".join(x) if isinstance(x, list) and all(isinstance(i, str) for i in x) else (str(x)[:200] + '...' if pd.notna(x) and isinstance(x, str) and len(str(x)) > 200 else (str(x) if pd.notna(x) else ''))
        )
    else:
        display_df.insert(len(display_df.columns), 'contexts', pd.NA) # Insertar columna si no existe
